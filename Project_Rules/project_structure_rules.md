...
1. **硬上限**：所有代码文件（除文档与 Swagger 产物）不得超过 `1000` 行（非空非注释）。
2. **推荐阈值**：普通逻辑文件（Provider/ViewModel/Service/Repository/Model/Utils）建议控制在 `500` 行以内。
3. **UI 体量**：UI 文件可适当放宽（Page/复合 Widget 建议 `800` 行以内），但同样不得超过 `1000` 行。
4. **可拆必拆/可复用必复用**：UI 中可拆分的组件必须拆分；可复用的组件必须抽取复用，避免复制粘贴式实现。
5. **职责上限**：单一逻辑/架构文件不得承担 `3` 个及以上功能域，职责上限为 `2` 个功能域。
6. **超限处理**：一旦超过 `1000` 行，必须先拆分再提交。
