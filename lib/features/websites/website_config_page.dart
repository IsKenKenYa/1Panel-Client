...
class WebsiteConfigProvider extends ChangeNotifier {
  ...
  WebsiteConfigProvider({required this.websiteId});
  ...
  Future<void> loadScope(NginxKey key) async {
    await _ensureApi();
    scope = key;
    final data = await _api!.loadWebsiteNginxConfig({
      'scope': key.value,
      'websiteId': websiteId,
    });
    scopeResponse = WebsiteNginxScopeResponse.fromJson(data);
    notifyListeners();
  }
  ...
  Future<void> updatePhpVersion(int? runtimeId) async {
    await _ensureApi();
    await _api!.updateWebsitePhpVersion(websiteId: websiteId, runtimeId: runtimeId);
  }
}
