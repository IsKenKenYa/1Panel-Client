...
| 图标 | 标签 | 功能描述 | 路由 |
|------|------|----------|------|
| 💻 | 服务器 | 服务器连接与分组管理 | `/servers` |
| 📁 | 文件管理 | 浏览和管理服务器文件 | `/files` |
| 🤖 | AI管理 | Ollama模型与MCP代理配置 | `/ai` |
| 🔧 | 设置 | 应用设置、Passkey和反馈 | `/settings` |
...
```
AI管理首页 → 显示本地大模型(Ollama)列表和GPU监控
          → 切换到MCP标签 → 查看/添加/编辑MCP代理配置
          → 切换到域名标签 → 为AI服务配置域名代理
          → 点击模型/代理项 → 查看详情和日志
```

**关键界面**: 
- **Ollama模型页**: 显示和管理本地大语言模型
- **GPU监控面板**: 实时展示显存使用和运行负载
- **MCP管理页**: 提供MCP Server代理环境的增删改查
- **域名代理页**: 管理AI相关的反向代理域名

### 3.9 多服务器管理模块流程

```
服务器列表 → 显示已保存的服务器连接列表(包含服务器名称、地址、状态)
           → 顶部切换分组标签 → 显示特定分组下的服务器
           → 点击服务器项 → 连接到服务器
           → 点击添加按钮 → 添加服务器向导
...
**关键界面**: 
- **服务器列表页**: 显示已保存的服务器连接列表，支持按分组筛选、快速切换和管理
- **分组管理页**: 管理服务器的分组信息
- **添加/编辑服务器页**: 表单用于输入服务器连接信息

### 3.10 设置模块流程

```
设置首页 → 显示设置分类列表 (系统设置、API配置、API测试、反馈中心等)
        → 点击分类项 → 进入对应设置页面
...
**关键界面**: 
- **设置首页**: 显示所有设置分类
- **通用设置页**: 包含主题、语言、通知、Passkey等基础设置
- **反馈中心页**: 提供问题反馈模板与日志一键导出功能（带隐私脱敏机制）
