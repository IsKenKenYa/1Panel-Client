...
## 10. CronjobFormPage 的文案和错误提示现在走哪条链路？

- 表单字段标签、分段标题、confirm 文案统一走 l10n
- `CronjobFormService` 抛出的表单错误改为稳定 error code
- 页面层通过 error code -> l10n 映射输出用户可见错误，避免 Week 5 新页继续暴露硬编码英文提示
- backup type / database type / alert method 的未知值也不会再直接显示原始后端字符串，而是落到 `CronjobForm` 模块级 fallback 文案
