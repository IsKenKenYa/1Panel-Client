...
### Q3: Week 1 已支持哪些主机相关能力？

**A**: Week 1 主要完成了主机资产 API 对齐和共享分组底座准备，具体包括：
1. 创建、更新、删除、搜索主机
2. `POST /core/hosts/info` 获取详情
3. `POST /core/hosts/test/byid/:id` 与 `POST /core/hosts/test/byinfo` 测试连接
4. `POST /core/hosts/update/group` 更新主机分组
...
**A**: 不是。
- 主机资产属于 Phase 1 Week 2
- 监控数据属于独立监控链路，不再混入 `host_v2.dart` 的主机资产闭环
...
**A**: 支持主机分组接口，移动端 UI 会在 Week 2 通过共享 `GroupSelectorSheet` 接入。
