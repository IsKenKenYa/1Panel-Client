...
- S2-5 当前进展：
  ...
  - `file`：`FilesService` 已拆分为 `FilesApiGateway + FileBrowserService / FileRecycleService / FileTransferService / FilePreviewService`；`FilesProvider` 拆分为多 part（`lifecycle / browser / recycle / favorites-transfer / system`），并新增 `RecycleBinProvider / TransferManagerProvider / FilePreviewProvider` 承接页面状态。
...
- 本轮门禁：`flutter analyze`、`dart run test_runner.dart unit`、`dart run test_runner.dart ui`、`dart run test_runner.dart integration` 全部通过（integration 含预期 skip）。
...
- Phase 1 主计划：
  `docs/development/modules/阶段1计划.md`
- 需求跟踪矩阵：
