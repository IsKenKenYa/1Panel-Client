...
- S2-4 启动增量：
  - Orchestration：新增 Repository/Service 层，compose/image/network/volume provider 去 API 直连。
  - AI：provider 去 API 直连（改依赖 repository），新增 `AppRoutes.ai`，并接入 Shell 与 ServerDetail 入口。
