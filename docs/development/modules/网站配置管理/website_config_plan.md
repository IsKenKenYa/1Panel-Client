...
### 页面结构
- 配置总览页: 当前配置状态、最近更新时间、风险提示。
- 配置编辑页: 只读/编辑切换、语法校验、差异预览、保存与回滚。
- 高级配置页: scope 选择器 + 结构化参数展示。
- PHP 版本页: 当前版本、可切换列表、风险说明。

### MDUI3 设计要点
- 使用卡片分组配置块，主要操作放在卡片头部操作区。
- 编辑器采用等宽字体与行号，触控目标 ≥ 44-48px。
- 保存与危险操作使用确认对话框与错误恢复入口。
