...
| 平台 | 状态 | 说明 |
|------|------|------|
| Android | ✅ 支持 | 原生 UI 默认，支持切换 MDUI3 |
| iOS / iPadOS | ✅ 支持 | 原生 UI 默认，支持切换 MDUI3 |
| macOS | ✅ 支持 | 原生 UI 默认，支持切换 MDUI3 |
| Windows | ✅ 支持 | 原生 UI 默认，支持切换 MDUI3 |
| Linux | ✅ 支持 | 原生 UI 默认，支持切换 MDUI3 |
| Web | ❌ 不在范围 | 本工作流默认不适配 Web |
| HarmonyOS（未来） | 🚧 占位 | 本阶段仅做 resolver/channel/provider 预留 |

### 双 UI 与平台适配策略（非 Web，强制）

- 非 Web 平台统一遵循“原生 UI 默认 + MDUI3 回退”策略。
- 原生 UI 与 MDUI3 必须共享 Dart 的 API/Model/Provider/Service/Repository/Infra 非表现层。
- 禁止出现“原生一套业务逻辑、MDUI3 一套业务逻辑”的双轨分叉。
- 若某平台原生壳能力未就绪，必须显式回退到 MDUI3 并记录回退原因与补齐计划。
- HarmonyOS 及其他未来平台，本阶段只允许 Dart 侧占位，不允许把共享业务核心下沉到原生层。
...
- [ ] 0.1 确认本次适配目标属于非 Web 平台（Android/iOS/macOS/Windows/Linux）
- [ ] 0.2 确认本次模块遵循“原生 UI 默认 + MDUI3 回退”并已定义回退条件
- [ ] 0.3 若涉及未来平台（如 HarmonyOS），仅落地 resolver/channel/provider 占位
- [ ] 1. 运行 `check_module_api_updates.py` 检查模块 API 是否有新增/删除
- [ ] 1.1 运行 `check_module_client_coverage.py` 检查 Swagger 与客户端覆盖差异
- [ ] 2. 对目标模块运行 `analyze_module_api.py` 获取最新API文档
- [ ] 3. 检查工作区内是否已存在类似实现（避免重复）
- [ ] 4. 检查存储架构文档 `docs/development/storage/`
- [ ] 5. 分析现有UI链路（组件、状态、样式、性能、代码质量）
...
- [ ] 13.1 执行非 Web 平台验证矩阵（至少覆盖 Native 与 MDUI3 两种模式）
- [ ] 14. 更新模块文档
