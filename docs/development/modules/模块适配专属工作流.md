...
- 本工作流默认只服务 **V2 适配**
- 若需要调研 **V1 历史接口**，应优先使用 **1Panel 子模块切换到 1.x 分支或 tag** 的方式获取当时的源码与 Swagger，而不是继续维护一份新的 V1/V2 双轨镜像
- 当前本地子模块已可见的 1.x 参考来源包括：
  - 远端分支：`origin/release-1.0` 到 `origin/release-1.10`
  - 历史 tag：`v1.2.0` 到 `v1.10.34-lts`
- 结论：
  - **V2 适配**：直接查看当前子模块 `swagger.json`
  - **V1 历史调研**：临时切换子模块到 1.x 分支/tag 后查看对应 `swagger.json`
...
> 注意：脚本默认读取**当前子模块检出的 Swagger**。如果你把子模块临时切到 1.x 分支/tag，脚本读到的也会是对应版本的规范。
...
# 临时切到某个 1.x tag 调研
git -C docs/OpenSource/1Panel checkout v1.10.34-lts

# 读取该版本 swagger
python3 analyze_module_api.py website /tmp/website_v1_check

# 调研完成后切回当前 V2 开发基线
git -C docs/OpenSource/1Panel checkout dev-v2
git -C docs/OpenSource/1Panel submodule update --init --recursive
```

> 如果你只是做当前分支的功能适配，请不要切换子模块版本。
...
当子模块更新后，建议先检查当前模块分析文件是否已经落后于最新 Swagger。
...
**推荐流程**:
1. 更新 `docs/OpenSource/1Panel` 子模块
