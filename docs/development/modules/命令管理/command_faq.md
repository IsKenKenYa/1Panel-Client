...
### 7. 为什么客户端对命令树/命令列表坚持用 POST，而不是按 Swagger 的 GET？

- 我们发现了契约偏差：Swagger 快照与后端运行时路由存在不一致。
- 命令模块按“运行时可用真值”执行：`tree/list` 使用 `POST`。
- 为避免掩盖问题，客户端不做 `GET` 回退；冲突通过 issue 闭环推进上游修复。
- 相关证据与编号会回填到模块计划文档中。
