...
1. 通过浏览器登录您的 1Panel Web 面板。
2. 进入左侧菜单的 **面板设置** -> **API 接口**。
3. 打开“API 接口”开关以**启用**访问。
4. 复制生成的 **API 密钥 (API Key)**。
5. **关键步骤**：配置 IP 白名单。如果您使用手机等移动网络（IP 会频繁变动），请在“允许的 IP”中填写 `0.0.0.0/0` 以允许所有 IP 访问。如果您有固定公网 IP 或 VPN，请填写您的具体 IP。

### 安全提示

- **使用 HTTPS**：生产环境强烈建议使用 HTTPS 访问。
- **IP 白名单**：尽可能限制特定的 IP 访问。如果必须配置为 `0.0.0.0/0`，请务必妥善保管好您的 API 密钥，不要泄露。
- **API 密钥**：保持 API 密钥安全并定期在面板中重新生成。
- **网络**：如果条件允许，优先使用 VPN 或内网进行远程管理连接。
