...
void main() {
  ...
  setUp(() async {
    ...
    client = DioClient(
        baseUrl: TestEnvironment.baseUrl, apiKey: TestEnvironment.apiKey);
    api = AppV2Api(client);
  });

  test('Verify App Icon Endpoint', () async {
    // 1. Search for OpenResty to get ID and Key
    final search = await api
        .searchApps(AppSearchRequest(page: 1, pageSize: 1, name: 'openresty'));
    ...
    try {
      ...
      debugPrint(
          'Icon by ID status: ${res1.statusCode}, Data length: ${res1.data?.length}');
    } catch (e) {
    ...
    try {
      ...
      debugPrint(
          'Icon by Key status: ${res2.statusCode}, Data length: ${res2.data?.length}');
    } catch (e) {
