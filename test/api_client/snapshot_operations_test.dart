...
void main() {
  ...
  group('快照操作API测试', () {
    test('POST /settings/snapshot/search - 搜索快照', () async {
      ...
      final response =
          await dio.post('/api/v2/settings/snapshot/search', data: {
        'page': 1,
        'pageSize': 10,
        'orderBy': 'createdAt',
        'order': 'descending',
    ...
    test('POST /settings/snapshot/recover - 恢复快照', () async {
      ...
      try {
        final response = await dio.post('/api/v2/settings/snapshot/recover',
            data: {'id': snapshotId});
    ...
    test('POST /settings/snapshot/rollback - 回滚快照', () async {
      ...
      try {
        final response = await dio.post('/api/v2/settings/snapshot/rollback',
            data: {'id': snapshotId});
        debugPrint('成功! 状态码: ${response.statusCode}');
        if (response.data != null) {
          final jsonStr =
    ...
    test('POST /settings/snapshot/description/update - 更新快照描述', () async {
      ...
      debugPrint('\n========================================');
      debugPrint(
          '测试: POST /settings/snapshot/description/update 更新描述 ID: $snapshotId');
      debugPrint('========================================');

      try {
        final response = await dio
            .post('/api/v2/settings/snapshot/description/update', data: {
          'id': snapshotId,
          'description': '测试描述更新',
        });
        debugPrint('成功! 状态码: ${response.statusCode}');
