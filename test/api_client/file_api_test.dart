...
void main() {
  ...
  group('文件检查 API 测试', () {
    test('POST /files/check - 检查文件是否存在', () async {
      ...
      try {
        ...
        debugPrint(
            '📥 文件类型: ${response.data?.isDirectory == true ? "目录" : "文件"}');
    ...
    test('POST /files/tree - 获取文件树', () async {
      ...
      try {
        ...
        if (response.data?.children != null &&
            response.data!.children!.isNotEmpty) {
          debugPrint('\n📁 子目录:');
          for (final child in response.data!.children!.take(5)) {
            debugPrint(
                '  - ${child.name} (${child.type == "dir" ? "目录" : "文件"})');
          }
        }
    ...
    test('使用expand参数获取文件列表', () async {
      ...
      try {
        ...
        if (data is Map) {
          ...
          if (data['items'] is List) {
            ...
            for (var i = 0;
              ...
              debugPrint(
                  '    [$i] ${item['name']} (${item['isDir'] == true ? '目录' : '文件'})');
            }
          }
        }
        ...
        if (parsed.items.isNotEmpty) {
          debugPrint('\n📁 前3个文件:');
          for (var i = 0;
              i < (parsed.items.length > 3 ? 3 : parsed.items.length);
              i++) {
  ...
  group('文件操作 API 测试', () {
    test('POST /files - 创建文件夹', () async {
      ...
      try {
        final testDir =
            '/tmp/test_flutter_api_${DateTime.now().millisecondsSinceEpoch}';
    ...
    test('POST /files/del - 删除文件', () async {
      ...
      try {
        final testDir =
            '/opt/1panel/test_delete_${DateTime.now().millisecondsSinceEpoch}';
  ...
  group('压缩解压 API 测试', () {
    test('POST /files/compress - 压缩文件', () async {
      ...
      try {
        ...
        try {
          await api
              .deleteFiles(FileBatchDelete(paths: [testFile, zipFile, dstDir]));
          debugPrint('🧹 清理测试文件');
        } catch (e) {
          debugPrint('⚠️ 清理失败: $e');
        }
  ...
  group('回收站恢复 API 测试', () {
    test('POST /files/recycle/reduce - 恢复回收站文件', () async {
      ...
      try {
        ...
        final recycleSearch = await api
            .searchRecycleBin(FileSearch(path: '/tmp', page: 1, pageSize: 50));
        ...
        if (recycleSearch.data != null) {
          for (final item in recycleSearch.data!) {
            if (item.name.contains('test_recycle_') &&
  ...
  group('文件高级功能 API 测试', () {
    ...
    test('POST /files/link/create - 创建链接', () async {
      ...
      try {
        ...
        await api.createFile(FileCreate(
            path: sourceFile, content: 'source content', isDir: false));
