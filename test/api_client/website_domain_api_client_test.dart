...
void _logSection(String title, {String? method, String? path, Object? request, Object? response}) {
  ...
  if (method != null && path != null) {
    appLogger.dWithPackage('test.api_client.website_domain', 'Request: $method $path');
  }
  if (request != null) {
    appLogger.dWithPackage('test.api_client.website_domain', 'RequestBody: ${_prettyJson(request)}');
  }
  if (response != null) {
    appLogger.dWithPackage('test.api_client.website_domain', 'Response: ${_prettyJson(response)}');
  }
  appLogger.dWithPackage('test.api_client.website_domain', '========================================');
}
...
void main() {
  ...
  group('网站域名管理 API客户端测试', () {
    ...
    test('GET /websites/domains/:websiteId 应该成功', () async {
      ...
      if (skipReason != null) {
        appLogger.wWithPackage('test.api_client.website_domain', '跳过测试: $skipReason');
        return;
      }

      final websites = await api.getWebsites(page: 1, pageSize: 1);
      if (websites.items.isEmpty) {
        appLogger.wWithPackage('test.api_client.website_domain', '测试服务器暂无网站，跳过域名拉取');
        return;
      }
    ...
    test('POST /websites/domains + update + del 应该成功', () async {
      ...
      final websites = await api.getWebsites(page: 1, pageSize: 1);
      if (websites.items.isEmpty) {
        appLogger.wWithPackage('test.api_client.website_domain', '测试服务器暂无网站，跳过域名新增/删除');
        return;
      }
      ...
      final testDomain = 'api-${DateTime.now().millisecondsSinceEpoch}.${TestEnvironment.testDomain}';
      int? createdDomainId;
      ...
      try {
        final rawCreate = await _rawPost(client, '/websites/domains', data: createRequest);
        _logSection('✅ Raw /websites/domains', method: 'POST', path: '/websites/domains', request: createRequest, response: rawCreate.data);
        await api.addWebsiteDomains(websiteId: websiteId!, domains: [{'domain': testDomain, 'port': port}]);
        createdDomainValue = testDomain;
      } catch (e) {
        appLogger.wWithPackage('test.api_client.website_domain', '/websites/domains 返回异常: $e');
      }

      final domains = await api.getWebsiteDomains(websiteId);
      if (domains.isEmpty) {
        appLogger.wWithPackage('test.api_client.website_domain', '域名列表为空，跳过更新/删除');
        return;
      }
      ...
      try {
        final rawUpdate = await _rawPost(client, '/websites/domains/update', data: updateRequest);
        _logSection('✅ Raw /websites/domains/update', method: 'POST', path: '/websites/domains/update', request: updateRequest, response: rawUpdate.data);
        await api.updateWebsiteDomainSsl(id: created.id!, ssl: created.ssl ?? false);
      } catch (e) {
        appLogger.wWithPackage('test.api_client.website_domain', '/websites/domains/update 返回异常: $e');
      }
      ...
      try {
        final rawDelete = await _rawPost(client, '/websites/domains/del', data: deleteRequest);
        _logSection('✅ Raw /websites/domains/del', method: 'POST', path: '/websites/domains/del', request: deleteRequest, response: rawDelete.data);
        await api.deleteWebsiteDomain(id: createdDomainId!);
      } catch (e) {
        appLogger.wWithPackage('test.api_client.website_domain', '/websites/domains/del 返回异常: $e');
      }
    });
  });
}
